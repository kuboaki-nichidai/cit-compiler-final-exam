...
【問2】カンマ区切りのCSVファイルの各行から、西暦の年月日を取り出す正規表現を作りなさい。
...
【準備C】作成した作用用ディレクトリへ、lox01のAssignmentのリポジトリからcloneしなさい。
...
NOTE: この文書に差し込んでいる下記ダミーファイルはJPEGファイルになっている。もしPNGを取得したなら、下記で指定しているファイル名の拡張子を変更しなさい。
...
Loxインタプリタの起動から、指定したプログラムを実行しているまでの様子を表示しているターミナルウィンドウのスナップを `images/sample_lox_snap.jpg` （PNGでもよい）に保存し、下記にスナップが表示されるようにしなさい。
...
【準備A】lox01の `codes/com/craftinginterpreters/lox` ディレクトリを内容ごとコピーして、 `codes/com/craftinginterpreters/lox_s` を作りなさい（ `lox` ディレクトリを内容ごとコピーして、 `lox_s` ディレクトリを作る）。

【準備B】 `lox_s` がGitHubに認識されるよう、`git add` コマンドで追加しておきなさい。

IMPORTANT: `add` を忘れると、push したときに反映されないため、採点対象にならないので注意すること。

【問6】Loxの文字列は「"」で囲まれた文字の並びであるが、文字列の中には「"」を含むことができない。
そこで、文字列内に「\"」のように書かれていた場合には、文字列の終わりとみなさず、文字列に含むことができるように修正しなさい。
...
print "abc\"defg"; // 今回修正して対応したい文字列の例
----

. `lox_s` 中のコードを修正して動作できるようにしなさい。
. 修正した`lox_s` 中のLoxインタプリタを動かしなさい。
. 修正できたら、実行時のスナップを `images/lox_s_snap.jpg` （またはPNG）と差し替えなさい。
